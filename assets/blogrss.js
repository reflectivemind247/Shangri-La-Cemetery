    ...
    function loadRssAsHtml(elId) {        
        fetch('https://reflectivemind247.github.io/Shangri-La-Cemetery/feed.xml') // Replace with your RSS feed URL
            ...
            items.forEach(item => {
                ...
                html += `<li class='blogListItem'><span class='blogItemDate'>${buildDate(newItem)}</span><span class='p button blogItemTitle'><a role='button' onclick='viewPost2(${rssItems.length});'><h3>${newItem.title}</h3></a></span><span class="p blogHighlight">${newItem.content.substring(0, 50)}...</span></li>`;
            });
            html += "</ul>";
